...
# Initialize count of detected objects and frameCount
detected = []
frameCount = 0
...
while True:
    ...
    (rects, _) = hog.detectMultiScale(
        frame, winStride=(4, 4), padding=(3, 3), scale=1.3)
    ...
    # Append detected objects for each frame
    frameCount += 1
    detected.append([frameCount, len(valid_cntrs), len(cars), len(rects)])
...
# Write to csv-file number detected objects
output_file = 'detected.csv'
with open(output_file, 'w', newline='') as output_file:
    fieldnames = ['frame_count', 'moving_obj', 'cars', 'pedestrians']
    output_writer = csv.writer(output_file, fieldnames)
